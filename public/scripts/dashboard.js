...
var x=setInterval(function(){
    if(stateIII==true){
        document.getElementById('l3').setAttribute('style','color:#33ccff;transition:1s;');
        document.getElementById('l2').setAttribute('style','color:#ff9933;transition:1s;');
        document.getElementById('l1').setAttribute('style','color:#99ff66;transition:1s;');
        stateIII=false;
    }
    else if(stateIII==false){
        document.getElementById('l3').setAttribute('style','color:#6633ff;transition:1s;');
        document.getElementById('l2').setAttribute('style','color:#ffff66;transition:1s;');
        document.getElementById('l1').setAttribute('style','color:#33ff99;transition:1s;');
        stateIII=true;
    }
},1000);
...
function Delete(url,index){
    fetch(`/${url}/delete-message`,{method:'PUT'}).then(function(res){
        res.json('/');
        document.getElementById(`message${index}`).remove();
